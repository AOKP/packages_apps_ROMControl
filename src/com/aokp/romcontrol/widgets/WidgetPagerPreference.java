    ...
    /*BroadcastReceiver mWidgetDataReceiver = new BroadcastReceiver() {
        public void onReceive(Context context, Intent intent) {
        	...
        	if (target > -1){
        		...
        		Log.d(TAG,"Rec'd:" + mWidgetIds[target] +" Provider:" + mProvider[target] + " Label:"+ mTitles[target]);
        	}      
    ...
    public void inflateWidgetPref() {
     ...
     public class WidgetPagerAdapter extends PagerAdapter {
         ...
         public Object instantiateItem(View collection, int position) {
             ...
             /*} else if (mWidgetResId[position] != 0) {
            	 PackageManager pm = mContext.getPackageManager();
            	 ComponentName cn = ComponentName.unflattenFromString(mProvider[position]);
                 mWidgetView.setImageDrawable(pm.getDrawable(cn.getPackageName(), mWidgetResId[position], null));           	 
             }*/ 
                 else {
