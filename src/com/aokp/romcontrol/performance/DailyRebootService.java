...
public class DailyRebootService extends IntentService {
    ...
    public DailyRebootService() {
        super("DailyRebootService");
    }

    @Override
    public void onCreate() {
        super.onCreate();
        am = (AlarmManager) getApplicationContext().getSystemService(
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        if (isDailyRebootEnabled(getApplicationContext())) {
            if (intent.hasExtra("reschedule")) {
                if (rescheduledCount > 4) {
                    log("too many reschedule attempts, not rebooting today");
                    // reboot tomorrow!
                    int[] rebootTime = getUserSpecifiedRebootTime(getApplicationContext());
    ...
    private void scheduleReboot(long when) {
        mRebootPendingIntent = PendingIntent.getBroadcast(getApplicationContext(), 0,
